...
def neighborhood(input_minimum_production, input_initial_state, input_initial_net_reserves,
                 input_maintenance_units_names):
    minimum_production = input_minimum_production
    ...
    for i in range(len(initial_state)):
        ...
        if (initial_state[min_net_reserves_index] < initial_state[i] and
            ...
            neighbour_production[min_net_reserves_index], neighbour_production[i] = neighbour_production[i], \
                                                                                    neighbour_production[
                                                                                        min_net_reserves_index]

            neighbour_maintenance_units_names[min_net_reserves_index], neighbour_maintenance_units_names[i] = \
            neighbour_maintenance_units_names[i], neighbour_maintenance_units_names[min_net_reserves_index]
...
def fitness(input_neighbour_net_reserves, input_initial_net_reserves, input_initial_state , input_temp):
    ...
    if (min_neighbour_net_reserves > min_initial_net_reserves):
        ...
        temp = input_temp
        fs = min_initial_net_reserves
        ft = min_neighbour_net_reserves
        print("fs:",fs)
        print("ft:",ft)
        print("temp:",temp)

        boltzmann = math.exp(- abs(fs - ft) / temp)

        print("boltz man:" ,boltzmann)

        random_num = random.uniform(0, 1)

        print("random_num:",random_num)

        if(0 < random_num and random_num < boltzmann and boltzmann != 1):
...
def barplot(input_interval_numbers, input_minimum_production, input_net_reserves, input_maintenance_units,
            input_units_capacity):
    # Data
    r = input_interval_numbers
    raw_data = {'minimum_production': input_minimum_production, 'net_reserves': input_net_reserves,
                'maintenance_units': input_maintenance_units}
    ...
    # Create orange Bars
    plt.bar(r, net_reserves, bottom=minimum_production, color='#f9bc86', edgecolor='white', width=barWidth,
            label="net reserves")
    # Create blue Bars
    plt.bar(r, maintenance_units, bottom=[i + j for i, j in zip(minimum_production, net_reserves)], color='#bfc7c7',
            edgecolor='white',
...
for i in range(0, generation):
    ...
    while (while_condition):
        ...
        neighbor_info = neighborhood(primary_info[1], intial_state_temp, intial_net_temp,
                                     initial_maintenance_units_names_temp)
        ...
        fitness_info = fitness(neighbor_net_temp, intial_net_temp, intial_state_temp,temp)
